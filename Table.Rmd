...
```{r}
kable(cbind(single_table(NSCH$SC_AGE_YEARS)),caption = "Age of Selected Child - In Years")
```

# Gender of Children:SC_SEX
```{r}
kable(cbind(single_table(NSCH$SC_SEX)),caption = "Gender of Children, Detailed")

```

# Race of Children:SC_RACE_R
```{r}
kable(cbind(single_table(NSCH$SC_RACE_R)),caption = "Race of Children, detailed")
```

# Hispanic Children:SC_HISPANIC_R
```{r}
kable(cbind(single_table(NSCH$SC_HISPANIC_R)),caption = "Hispanic Origin of Selected Child")
```

# Special Health needs: SC_CSHCN
```{r}
kable(cbind(single_table(NSCH$SC_CSHCN)),caption = "Special Health Care Needs Status of Selected Child")
```

# Highest Level of Education among Reported Adults: HIGRADE
```{r}
kable(cbind(single_table(NSCH$HIGRADE)),caption = "Highest Level of Education among Reported Adults")
```

# sex of children? : sex_1920
```{r}
kable(cbind(single_table(NSCH$sex_1920)),caption = "sex of children")
```
...
```{r}
kable(cbind(single_table(NSCH$PrntNativity_1920)),caption = "Generational status of the parents")
```
...
```{r}
kable(cbind(single_table(NSCH$povlev4_1920)),caption = "Income based on federal poverty level status")
```
...
```{r}
kable(cbind(single_table(NSCH$NbhdSafe_1920)),caption = "Indicator 7.2: Children live in safe neighborhoods")
```
...
```{r}
kable(cbind(single_table(NSCH$NbhdAmenities_1920)),caption = "Indicator 7.4: Count of amenities in children's neighborhoods")
```
...
```{r}
kable(cbind(single_table(NSCH$DevScrnng_1920)),caption = "Screen")
```
